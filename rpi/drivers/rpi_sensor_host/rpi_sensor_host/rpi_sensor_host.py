...
class RpiSensorHost(RpiSensor):
    ...
    def uptime(cls):
        ...
        return RpiSensorBuilder.make_float_sensor(inspect.currentframe().f_code.co_name, uptime, 2, SensorDeviceClass.DURATION)

    @classmethod
    def cpu_usage(cls) -> dict:
        usage = 100.0 - get_float_field_from_proc("mpstat", -1, -1)
        return RpiSensorBuilder.make_float_sensor(inspect.currentframe().f_code.co_name, usage, 2, PERCENTAGE)

    @classmethod
    def cpu_temperature(cls) -> dict:
        temperature = get_float_field_from_proc(["cat", "/sys/class/thermal/thermal_zone0/temp"], 0, 0) / 1000.0
        return RpiSensorBuilder.make_float_sensor(inspect.currentframe().f_code.co_name, temperature, 3, SensorDeviceClass.TEMPERATURE)

    @classmethod
    def memory_usage(cls):
        fields = get_fields_from_proc(["free", "-bw"], 1)
        total = float(fields[1])
        return RpiSensorBuilder.make_float_sensor(inspect.currentframe().f_code.co_name, (100.0 * (total - float(fields[-1]))) / total, 2, PERCENTAGE)

    @classmethod
    def swap_usage(cls):
        fields = get_fields_from_proc(["free", "-bw"], 2)
        return RpiSensorBuilder.make_float_sensor(inspect.currentframe().f_code.co_name, (100.0 * float(fields[2])) / float(fields[1]), 2, PERCENTAGE)
